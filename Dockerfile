# syntax=docker/dockerfile:1.7

ARG OPENCLAW_EXTENSIONS=""
ARG OPENCLAW_VARIANT=default
ARG OPENCLAW_BUNDLED_PLUGIN_DIR=extensions
ARG OPENCLAW_DOCKER_APT_UPGRADE=1
ARG OPENCLAW_NODE_BOOKWORM_IMAGE="node:24-bookworm@sha256:3a09aa6354567619221ef6c45a5051b671f953f0a1924d1f819ffb236e520e6b"

# --- Stage 1: Build ---
FROM ${OPENCLAW_NODE_BOOKWORM_IMAGE} AS build
WORKDIR /app

# Install Bun & Corepack
RUN curl -fsSL https://bun.sh/install | bash
ENV PATH="/root/.bun/bin:${PATH}"
RUN corepack enable

# Copy only what's needed for install
COPY package.json pnpm-lock.yaml pnpm-workspace.yaml .npmrc ./
COPY ui/package.json ./ui/package.json
COPY patches ./patches
COPY scripts/postinstall-bundled-plugins.mjs ./scripts/postinstall-bundled-plugins.mjs

# Install dependencies (no cache mounts for Railway)
RUN NODE_OPTIONS=--max-old-space-size=1536 pnpm install --frozen-lockfile

# Copy the rest of the source
COPY . .

# --- Stage 2: Runtime ---
FROM ${OPENCLAW_NODE_BOOKWORM_IMAGE} AS runtime
WORKDIR /app

# System packages
RUN apt-get update && apt-get install -y --no-install-recommends \
      procps hostname curl git lsof openssl && rm -rf /var/lib/apt/lists/*

# Copy build artifacts and necessary source files
COPY --from=build /app/dist ./dist
COPY --from=build /app/node_modules ./node_modules
COPY --from=build /app/package.json ./
COPY --from=build /app/openclaw.mjs ./

# Environment
ENV NODE_ENV=production

# Make CLI available & set permissions
RUN ln -sf /app/openclaw.mjs /usr/local/bin/openclaw && chmod 755 /app/openclaw.mjs
RUN chown -R node:node /app
USER node

# Health check
HEALTHCHECK --interval=3m --timeout=10s --start-period=30s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:18789/healthz').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"

# Default CMD to run the gateway
CMD ["sh", "-c", "node openclaw.mjs gateway --allow-unconfigured --bind lan --port $PORT"]
